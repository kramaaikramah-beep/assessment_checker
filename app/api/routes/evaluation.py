...
def _validate_answer_sheet(filename: str) -> None:
    if Path(filename).suffix.lower() != ".docx":
        raise HTTPException(
            status_code=400,
            detail="Answer sheet must be uploaded as a .docx file for comparison mode.",
        )


@router.post("/evaluate", response_model=EvaluationResponse)
async def evaluate(file: UploadFile = File(...), answer_sheet: UploadFile | None = File(None)):
    UPLOAD_DIR.mkdir(parents=True, exist_ok=True)
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

    original_name = Path(file.filename or "submission.docx").name
    _validate_upload(original_name, "Trial build accepts .docx files only.")
    ...
    try:
        result = run_pipeline(input_path, output_path, answer_sheet_path=answer_sheet_path)
    ...
    return EvaluationResponse(
        ...
        unit_file=result["unit_file"],
        answer_sheet_filename=result.get("answer_sheet_filename"),
        answer_sheet_source=result.get("answer_sheet_source", "configured_knowledge_base"),
